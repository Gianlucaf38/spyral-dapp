...
describe("SongAsset Oracle Mock Test", function () {
  ...
  it("Mint → Request → Oracle callback", async function () {
    ...
    // 1 mint (conia il token assegnandolo a 'user')
    ...
    // 4 spotify id (DEVE essere chiamato da 'user' che possiede l'NFT)
    await contract.connect(user).setSpotifyId(1, "TEST");

    // 5 request oracle (Chiamato dall'owner del contratto, corretto)
  ...
  it("Stream milestone triggers revenue", async function () {
    ...
    // CORRETTO: Aggiunto connect(user) per l'autorizzazione sull'NFT
    await contract.connect(user).setSpotifyId(1, "TEST");
